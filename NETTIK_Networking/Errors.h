...
#define GEN_ERROR( sError )\
	throw gen_net::CFatalException( (sError), __FILE__, __LINE__ );
...
#define GEN_ASSERT( bCondition, sError )\
	if (!(bCondition)) { throw gen_net::CFatalException( (sError), __FILE__, __LINE__ ); }
...
#define GEN_CATCHGUARD\
	catch( gen_net::CFatalException& e ) { e.AppendToCallStack( __FUNCTION__, ObjectName() ); throw e; }\
	catch( ... ) { throw gen_net::CFatalException( __FILE__, __FUNCTION__, ObjectName() ); }
...
#define GEN_ENDSENTRY\
	...
	catch( gen_net::CFatalException& e )\
	{ e.AppendToCallStack( __FUNCTION__, ObjectName(), true ); e.Display(); exit( EXIT_FAILURE ); }\
	catch( ... )\
	{ gen_net::CFatalException e( "Unknown Exception", __FILE__, __LINE__ ); \
	  e.AppendToCallStack( __FUNCTION__, ObjectName(), true ); e.Display(); exit( EXIT_FAILURE ); }
