...
public:
	...
	bool IsZero() const
	{
		return gen_net::IsZero( w*w + x*x + y*y + z*z );
	}

	// Test if the quaternion is the identity (i.e. = (1,0,0,0))
	bool IsIdentity() const
	{
		return gen_net::AreEqual( w, 1.0f ) &&
			gen_net::IsZero( x ) && gen_net::IsZero( y ) && gen_net::IsZero( z );
	}

	// Test if the quaternion is unit length (normalised)
	bool IsUnit() const
	{
		return gen_net::IsZero( w*w + x*x + y*y + z*z - 1.0f );
	}
