...
V1.2    Added precompiled header usage -JB
**************************************************************************************************/
...
namespace gen_net
{
	...
	)
	{
		...
		GEN_ASSERT(!gen_net::IsZero(scaleX) && !gen_net::IsZero(scaleY) && !gen_net::IsZero(scaleZ),
	...
	void CQuaternion::Normalise()
	{
		...
		if (gen_net::IsZero(fNormSquared))
		{
			w = x = y = z = 0.0f;
		}
		else
	...
	)
	{
		...
		if (cosTheta >= 0.0f)
		{
			...
			if (!AreEqual(cosTheta, 1.0f))
			{
				...
				slerp = (sin(t_inv) * theta) * p + sin(t * theta) * q / sin(theta);
			}
			else
			{
		...
		else
		{
			...
			if (!AreEqual(cosTheta, -1.0f))
			{
				...
				slerp = (sin(t - 1.0f) * theta) * p + sin(t * theta) * q / sin(theta);
			}
			else
			{
				slerp = (p * t_for) + (q * t);
